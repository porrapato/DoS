// DoS_70.cpp
// Arquivo de ataque DoS, com 50% de usuários maliciosos, duração do ataque de 60s.
...
main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < 9; i++) {
    ...
    consumer.Stop(Seconds(60 - i/2)); // stop consumers at 25s, 26s, 27s, 28s, 29s and 30s
  }

  for (int i = 9; i < 18; i++) {
    ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr");
    consumerHelper.SetAttribute("Frequency", StringValue("512")); // 512 interests a second
    ...
    consumer.Stop(Seconds(60 - i/4)); // stop consumers at 27s, 28s, 29s and 30s
  }
